...
describe('GET /projects', () => {
  test('It should get the project list', async () => {
    ...
    expect(response.body.data[0].employees[0].employeeId.firstName).toBe(
      'Roman',
    );
  });
  test('It should get status 404', async () => {
    ...
    expect(response.body.message).toBe(
      'Project not found with those parameters',
    );
  ...
  test('It should get populate employee', async () => {
    const response = await request(app).get('/projects');
    expect(response.body.data[0].employees[0].employeeId._id).toBe(
      '6289016df4d67ad85b52d9af',
    );
    expect(response.body.data[0].employees[0].employeeId.firstName).toBe(
      'Roman',
    );
    expect(response.body.data[0].employees[0].employeeId.lastName).toBe(
      'Riquelme',
    );
  });
});
describe('GET /:id', () => {
  ...
  test('It should get an array of one populate', async () => {
    const response = await request(app).get(`/projects/${projectId}`).send();
    expect(response.body.data.employees[0].employeeId._id).toBe(
      '62840603549ef329a075ef63',
    );
    expect(response.body.data.employees[0].employeeId.firstName).toBe(
      'Luciano',
    );
  ...
  test('It should return a status 404', async () => {
    const response = await request(app)
      .get('/projects/628a59c9f67d451615818847')
      .send();
  ...
  test('It should return a status 500', async () => {
    ...
    expect(response.body.message).toBe(
      'Cast to ObjectId failed for value "6" (type string) at path "_id" for model "Project"',
    );
...
describe('PUT /project', () => {
  ...
  test('It should get status 500 and employee ID invalid', async () => {
    ...
    expect(response.body.message).toBe(
      'Cast to embedded failed for value "{ rate: 12, role: \'PM\', employeeId: \'62840603549ef329a075ef6\' }" (type Object) at path "employees" because of "CastError"',
    );
    expect(response.statusCode).toBe(500);
    expect(response.body.data).toBe(undefined);
    expect(response.body.error).toBe(true);
...
describe('DELETE /projects/:id', () => {
  ...
  test('It should return status 404', async () => {
    const response = await request(app)
      .delete('/projects/628a59c9f67d45161581886a')
      .send();
    expect(response.body.message).toBe('Project not found');
    expect(response.statusCode).toBe(404);
    expect(response.body.error).toBe(true);
